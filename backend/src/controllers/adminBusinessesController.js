...
exports.listBusinesses = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT id, brand_name, contact_email, phone, custom_domain, created_at FROM businesses');
...
exports.createBusiness = async (req, res) => {
  try {
    ...
    const { brand_name, contact_email, phone, custom_domain } = req.body || {};
    if (!brand_name || !contact_email) return res.status(400).json({ message: 'brand_name e contact_email são obrigatórios' });

    const [result] = await pool.query('INSERT INTO businesses (id, brand_name, contact_email, phone, custom_domain) VALUES (UUID(), ?, ?, ?, ?)', [brand_name, contact_email, phone || null, custom_domain || null]);
    const [rows] = await pool.query('SELECT id, brand_name, contact_email, phone, custom_domain, created_at FROM businesses WHERE id = ?', [result.insertId]);
    
    const [byEmail] = await pool.query('SELECT id, brand_name, contact_email, phone, custom_domain, created_at FROM businesses WHERE contact_email = ? LIMIT 1', [contact_email]);
...
exports.updateBusiness = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT id, brand_name, contact_email, phone, custom_domain, created_at FROM businesses WHERE id = ? LIMIT 1', [id]);
